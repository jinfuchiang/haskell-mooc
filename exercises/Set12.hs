...
instance Functor Result where
  fmap f (MkResult res) = MkResult (f res)
  fmap f NoResult = NoResult
  fmap f (Failure info) = Failure info
...
instance Functor List where
  fmap f Empty = Empty
  fmap f (LNode x list) = LNode (f x) (fmap f list)
...
instance Functor TwoList where
    ...
    fmap f (TwoNode x y list) = TwoNode (f x) (f y) (fmap f list)
...
count v = foldr (\x y->if x==v then y+1 else y) 0
...
instance Foldable List where
  foldr f def Empty = def
  foldr f def (LNode v list) = f v (foldr f def list)
...
instance Foldable TwoList where
  foldr f def TwoEmpty = def
  foldr f def (TwoNode x y list) = f x $ f y (foldr f def list)
