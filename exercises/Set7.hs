...
data State = Start | Error | Finished | FlourOrSugarNeeded | FlourNeeded | SugarNeeded | MixNeeded | BakeNeeded
  deriving (Eq,Show)

step Start AddEggs = FlourOrSugarNeeded
step FlourOrSugarNeeded AddFlour = SugarNeeded
step FlourOrSugarNeeded AddSugar = FlourNeeded
step FlourNeeded AddFlour = MixNeeded
step SugarNeeded AddSugar = MixNeeded
step MixNeeded Mix = BakeNeeded
step BakeNeeded Bake = Finished
...
reverseNonEmpty (x:|xs) = case reverse (x:xs) of (x:xs) -> x:|xs
...
instance Semigroup Distance where
  (<>) (Distance l) (Distance r) = Distance (l + r)

instance Semigroup Time where
  (<>) (Time l) (Time r) = Time (l + r)

instance Semigroup Velocity where
  (<>) (Velocity l) (Velocity r) = Velocity (l + r)
...
passwordAllowed password (MinimumLength minLength) = minLength <= (length password)
passwordAllowed password (ContainsSome str) = any (\c->elem c password) str
passwordAllowed password (DoesNotContain str) = all (\c->not (elem c password)) str
-- passwordAllowed pw (DoesNotContain chars) = not (passwordAllowed pw (ContainsSome chars))
passwordAllowed password (And pr1 pr2) = passwordAllowed password pr1 && passwordAllowed password pr2
passwordAllowed password (Or pr1 pr2) = passwordAllowed password pr1 || passwordAllowed password pr2
...
data Arithmetic = Addition Arithmetic Arithmetic 
...
operation "+" l r = Addition l r
operation "*" l r = Multiplication l r

evaluate :: Arithmetic -> Integer
evaluate (Literal i) = i
evaluate (Addition l r) = evaluate l + evaluate r
evaluate (Multiplication l r) = evaluate l * evaluate r

render :: Arithmetic -> String
render (Literal i) = show i
render (Addition l r) = "(" ++ render l ++ "+" ++ render r ++ ")"
render (Multiplication l r) = "(" ++ render l ++ "*" ++ render r ++ ")"