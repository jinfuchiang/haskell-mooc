...
up position = MkPosition (x position) (1 + y position)

-- right increases the x value of a position by one
right :: Position -> Position
right position = MkPosition (1 + x position) (y position)
...
get :: UpDown -> Int
get (IncreasingMode n) = n
get (DecreasingMode n) = n

-- tick increases an increasing counter by one or decreases a
-- decreasing counter by one
tick :: UpDown -> UpDown
tick (IncreasingMode n) = IncreasingMode (n+1)
tick (DecreasingMode n) = DecreasingMode (n-1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (IncreasingMode n) = DecreasingMode n
toggle (DecreasingMode n) = IncreasingMode n
...
rgb Green = [0,1,0]
rgb Blue = [0,0,1]
-- why this one not work : rgb (Mix x y) = zipWith ((/2).(+)) (rgb x) (rgb y)
rgb (Mix x y) = map (/2) $ zipWith (+) (rgb x) (rgb y)
...
toNat z 
    | z < 0 = Nothing
    | z == 0 = Just Zero
    | z > 0 = case toNat (z-1) of Just x -> Just (PlusOne x)
-- toNat z
--   | z < 0      = Nothing
--   | otherwise  = Just (go z)
--   where go 0 = Zero
...
toBin :: Int -> Bin
toBin n = if n == 0 then O End else go n
  where 
    go n 
      | n == 0 = End
      | mod n 2 == 0 = O $ go (div n 2)
      | mod n 2 == 1 = I $ go (div n 2)
