...
sums :: Int -> [Int]
sums i = go 1 0 i
    where go i sum right
            | i > right = []
            | otherwise = sum + i : go (i+1) (sum+i) right
...
mymaximum bigger initial (x:xs) = if bigger initial x then mymaximum bigger initial xs
...
maybeMap f [] = []
maybeMap f (x:xs) = case f x of Just(value) -> value : maybeMap f xs
