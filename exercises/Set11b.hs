...
appendAll ioref xs = modifyIORef ioref (\str->foldl (++) str xs)
...
hSelectLines h nums = do
    lines <- hFetchLines h
    return [lines !! (num-1) | num <- nums]
    -- return $ map (\i -> lines !! (i-1)) nums
...
interact' f state = do
    line <- getLine
    let (isContinue, outputInfo, nState) = f (line, state)
    putStrLn outputInfo
    if isContinue then interact' f nState else return nState
